...
static SubTestRec _tinvalidFileTest1(int testNum, int subTest){
    ...
    if (response.type != INV_FILE || response.line != -1 || obj != NULL){
        sprintf(feedback, "Subtest %d.%d: Failed to get INV_FILE with NULL filename for createGEDCOM.",testNum,subTest);
...
static SubTestRec _tinvalidFileTest2(int testNum, int subTest){
    ...
    if (response.type != INV_FILE || response.line != -1 || obj != NULL){
        sprintf(feedback, "Subtest %d.%d: Failed to get INV_FILE with non-existing filename [%s] for createGEDCOM.", testNum, subTest, fileName);
        result = createSubResult(FAIL, feedback);
        return result;
    }
    else{
...
static SubTestRec _tinvalidFileTest3(int testNum, int subTest){
    ...
    if (response.type != INV_FILE || response.line != -1 || obj != NULL){
        sprintf(feedback, "Subtest %d.%d: Failed to get INV_FILE with non-.ged filename [%s] for createGEDCOM.", testNum, subTest, fileName);
        result = createSubResult(FAIL, feedback);
        return result;
    }
    else{
...
static SubTestRec _tinvalidFileTest4(int testNum, int subTest){
    ...
    if (response.type != INV_GEDCOM || response.line != -1 || obj != NULL){
        sprintf(feedback, "Subtest %d.%d: Failed to get INV_GEDCOM with missing header in file [%s] for createGEDCOM.", testNum, subTest, fileName);
        result = createSubResult(FAIL, feedback);
        return result;
    }
    else{
...
static SubTestRec _tinvalidFileTest5(int testNum, int subTest){
    ...
    if (response.type != INV_GEDCOM || response.line != -1 || obj != NULL){
        sprintf(feedback, "Subtest %d.%d: Failed to get INV_GEDCOM with missing submitter in file [%s] for createGEDCOM.", testNum, subTest, fileName);
        result = createSubResult(FAIL, feedback);
        return result;
    }
    else{
...
static SubTestRec _tinvalidFileTest12(int testNum, int subTest){
    ...
    if (response.type != INV_RECORD || response.line != 3 || obj != NULL){
        sprintf(feedback, "Subtest %d.%d: Failed to get INV_RECORD with > 255 line in file [%s] for createGEDCOM.", testNum, subTest, fileName);
        result = createSubResult(FAIL, feedback);
        return result;
    }
    else{
...
static SubTestRec _tinvalidFileTest13(int testNum, int subTest){
    ...
    if (response.type != INV_RECORD || response.line != 7 || obj != NULL){
